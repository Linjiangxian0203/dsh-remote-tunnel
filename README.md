...
| `audit` shows `in-use` rows marked `STALE` and ports stay "taken" | Those are historical rows left by sessions that were killed instead of `down`ed (allocation treats them as occupied to avoid a collision). Clean them with `dsh --profile remote audit <host> --clean-stale`. |
